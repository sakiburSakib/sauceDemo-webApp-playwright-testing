...
Call log:
  - waiting for locator('//button[contains(@class,"btn btn_primary")]').nth(4)
...
```ts
  ...
  10 |     }
  11 |     
  12 |     async addThreeRandomProducts() {
  13 | 
  14 |         const buttons = this.locate.addToCartButtons;
  15 | 
  16 |         const total = await buttons.count();
  17 | 
  18 |         if (total < 3) {
  19 |             throw new Error(`Not enough products found. Only ${total} buttons available`);
  20 |         }
  21 | 
  22 |         const used = new Set();
  23 | 
  24 |         while (used.size < 3) {
  25 | 
  26 |             const random = Math.floor(Math.random() * total);
  27 | 
  28 |             if (!used.has(random)) {
  29 |                 used.add(random);
  30 | 
  31 |                 const btn = buttons.nth(random);
  32 | 
> 33 |                 await btn.click();
     |                           ^ Error: locator.click: Test timeout of 30000ms exceeded.
  34 |             }
  ...
  37 | 
  38 |     async sortZtoA() {
  39 |         await this.locate.sortDropdown.selectOption('za');
  40 |     }
  41 | 
  42 |     async addFirstProductAfterSort() {
  43 | 
  44 |         const productName = await this.locate.inventoryItems
  45 |             .first()
  46 |             .textContent();
  47 | 
  48 |         await this.locate.addToCartButtons
  49 |             .first()
  50 |             .click();
  51 | 
  ...
  53 |     }
  54 | 
  55 |     async getAllProductNames() {
  56 |         return await this.locate.inventoryItems.allTextContents();
  57 |     }
  58 | 
  59 |     async openCart() {
  60 |         await this.locate.cartButton.click();
  61 |     }
  62 | }
```