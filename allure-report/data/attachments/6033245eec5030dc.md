...
Call log:
  - waiting for locator('//button[contains(@class,"btn btn_primary")]').nth(5)
...
```ts
  ...
  3  | import testData from '../../resource/testData.json' assert { type: 'json' };
  4  | 
  5  | export default class Inventory {
  6  | 
  7  |     constructor(page) {
  8  |         this.page = page;
  9  |         this.locate = new inventoryObject(page);
  10 |         this.utility = new Utilities(page);
  11 |     }
  12 | 
  13 |     async addThreeRandomProducts() {
  14 | 
  15 |         const total = await this.locate.addToCartButtons.count();
  16 |         const usedIndexes = [];
  17 | 
  18 |         while (usedIndexes.length < 3) {
  19 | 
  20 |             const random = await this.utility.getRandomNumber(0, total);
  21 | 
  22 |             if (!usedIndexes.includes(random)) {
  23 | 
  24 |                 usedIndexes.push(random);
  25 | 
  26 |                 await this.locate.addToCartButtons
  27 |                     .nth(random)
> 28 |                     .click();
     |                      ^ Error: locator.click: Test timeout of 30000ms exceeded.
  29 |             }
  30 |         }
  31 | 
  32 |         return usedIndexes;
  33 |     }
  34 | 
  35 |     async sortZtoA() {
  36 |         await this.locate.sortDropdown.selectOption({ label: 'Name (Z to A)' });
  37 |     }
  38 | 
  39 |     async addFirstProductAfterSort() {
  40 | 
  41 |         const productName =
  42 |             await this.locate.inventoryItems
  43 |                 .nth(0)
  44 |                 .textContent();
  45 | 
  46 |         await this.locate.addToCartButtons
  47 |             .nth(0)
  48 |             .click();
  49 | 
  ...
  51 |     }
  52 | 
  53 |     async getAllProductNames() {
  54 |         return await this.locate.inventoryItems.allTextContents();
  55 |     }
  56 | 
  57 |     async openCart() {
  58 |         await this.locate.cartButton.click();
  59 |     }
  60 | }
```